...
def init_springs():
    for i, j in ti.ndrange(N, N):
        ...
        if j < N - 2:
            idx_double_down = i * N + (j + 2)
            c = ti.atomic_add(num_springs[None], 1)
            spring_pairs[c] = ti.Vector([idx, idx_double_down])
            spring_lengths[c] = (x[idx] - x[idx_double_down]).norm()
            spring_stiffness[c] = k_s_bending
...
def main():
    ...
    window = ti.ui.Window("Mass-Spring System (Full Springs)", (800, 800))
