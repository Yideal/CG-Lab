...
def run_animation(model, betas, global_orient, body_pose, faces, out_dir, num_frames=30):
    ...
    try:
        ...
        print("  提示：未安装 imageio，无法生成 GIF 动画，请手动安装 `pip install imageio`")
...
def main(args):
    ...
    if args.animate:
        print("正在生成姿态动画...")
        run_animation(model, betas, global_orient, body_pose, faces, out_dir, num_frames=args.num_frames)
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        ...
        if args.animate:
            f.write(f"animation_frames: {args.num_frames}\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--animate", action="store_true", help="生成姿态动画")
    parser.add_argument("--num-frames", type=int, default=30, help="动画帧数")
